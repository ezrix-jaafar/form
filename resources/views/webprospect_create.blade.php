...
<div class="max-w-sm w-full bg-white rounded-lg shadow-md p-6">
    ...
    <form method="POST" action="/webprospect"  hx-post="/webprospect" hx-swap="outerHTML">
    ...
    <div class="relative mb-5">
        <label for="website_type" class="input-label hidden absolute top-0 left-3 -mt-1 bg-white px-1 text-gray-700 text-sm font-bold">Website Type</label>
        <select id="website_type" name="website_type" class="appearance-none border rounded w-full p-3 text-gray-700 leading-tight focus:outline-none focus:border-black">
            <option value="" disabled selected>Choose website type</option>
            @foreach (\App\Enums\Website::cases() as $sale)
                <option value="{{ $sale->value }}">{{ ucfirst($sale->value) }}</option>
            @endforeach
        </select>
    </div>
